...
class Game(SceneManager):
	def __init__(self, debug_level: int) -> None:
		...
		if self.debug:
			# Logging calls dispatched in a lambda (of which there is one (and which
			# is kinda completely decorative)), trigger an error since loguru interprets
			# the autogenerated "<lambda>" function name as a formatting tag bla bla
			# long story short change it here, no more error, presto pesto
			def lambda_patcher(record: "Record"):
				if record["function"] == "<lambda>":
					record["function"] = "~lambda"

			logger.configure(patcher=lambda_patcher)
	...
	# The two methods below are subject to extremely heavy change
