...
HEADER = """
ctypedef uint8_t (* FPTR_pyobj_extractor)(size_t size, object data_iterable, void *target) except 1
"""

EXTRACTOR_SKELETON = """
cdef uint8_t extract_{c_type_name}s(size_t size, object data_iterable, void *target) except 1:
	cdef {c_typedef} *cast_target = <{c_typedef} *>target
	cdef int64_t i = 0
	for i, it in enumerate(data_iterable):
		# print(f"Slammed", it, "into array at", <size_t>(cast_target + i))
		cast_target[i] = <{c_typedef}><{expected_py_type}{cast_safety}>it
	# print("Converted", i, "pyobjs")
	return 0
"""
...
def main():
	...
	with (
		generator_path / "vertexbuffer_pyobj_extractors.pyi"
